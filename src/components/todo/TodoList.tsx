...
export function TodoList() {
    const [tasks, setTasks] = React.useState<{ id: string; text: string; completed: boolean }[]>([])
    ...
    return (
        <>
            <ContextMenu>
            <Card className="h-[calc(100vh-8rem)] max-w-7xl w-full shadow-none border-none bg-transparent">
                    ...
                    <ScrollArea className="h-[calc(100vh-18rem)] pr-4">
                        <div className="space-y-3">
                            ...
                            {tasks.map((task) => (
                                <div
                                    key={task.id}
                                    className="group flex items-center justify-between p-3 rounded-xl border bg-card hover:bg-accent/50 transition-colors"
                                >
                                    <div className="flex items-center gap-3">
                                        ...
                                        />
                                        <span className={task.completed ? "line-through text-muted-fg" : ""}>{task.text}</span>
                                        <Click/>
                                    </div>
