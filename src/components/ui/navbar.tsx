...
// Inside src/components/ui/navbar.tsx
const NavbarTrigger = ({ className, onPress, ref, ...props }: NavbarTriggerProps) => {
    ...
    return (
        <Button
            ...
            variant="ghost" // or intent="plain" depending on which button lib you use
            aria-label={props["aria-label"] || "Toggle Navbar"}
            className={cn("-ms-2 lg:hidden", className)}
            // CHANGE THIS: Add onClick as a fallback for onPress
            onClick={(event) => {
                toggleNavbar()
            }}
            // If your Button component specifically expects onPress (React Aria), keep it,
            // but if it's a Shadcn button, it MUST be onClick.
            {...props}
        >
